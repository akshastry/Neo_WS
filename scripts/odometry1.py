...
def EKF():
	...
	# K_k = np.matmul(np.matmul(P_k, np.transpose(H_k)), np.linalg.inv(S_k))

	# print(S_k)
	# print("   ")
	# print(np.linalg.inv(S_k))
	# print("   ")
	# print(np.matmul(np.matmul(K_k, H_k), P_k))

	X_k 	= X_k + np.transpose(np.matmul(K_k, np.reshape(y_tilda, (-1, 1))))[0]
	# P_k		= P_k - np.matmul(np.matmul(K_k, H_k), P_k)

	# joseph stabilized form of update equation
...
def update_Kalman_mat():
	...
	F_k = I
	F_k[0, 1:4] = F_k[0, 1:4] + I_R_B[2, 0:3] * dt
	F_k[1:4, 1:4] = F_k[1:4, 1:4] - Omega_cross * dt

	G_k = np.zeros((4,3))
	G_k[0,0:3] = 0.5 * dt * dt * I_R_B[2, 0:3]
	G_k[1,0] = dt
	G_k[2,1] = dt
	G_k[3,2] = dt
	...
	H_k[0,1] = 1.0
	H_k[1,2] = 1.0
	H_k[2,0] = -1.0/(np.cos(phi) * np.cos(theta))
...
def main():
	global flow_x, flow_y, ground_distance_Lidar, main_running, Hz, X_k, Z_k, r_flow_sensor, Omega_cross, Lidar_offset_from_flow, \
	 dt, end_time, mass, g, Kp_x, Kp_y, Kp_z, Ki_x, Ki_y, Ki_z, Kd_z, err_sum_x, err_sum_y, err_sum_z
	...
	while not rospy.is_shutdown():
		...
		# derive control
		zdd = Kp_z * (Z_d - X_k[0]) + Kd_z * (0.0 - X_k[3]) + Ki_z * err_sum_z
		ydd = Kp_y * (0.0 - X_k[2]) + Ki_y * err_sum_y
		xdd = Kp_x * (0.0 - X_k[1]) + Ki_x * err_sum_x
		...
		err_sum_x = constrain(err_sum_x + (0.0 - X_k[1]), -1.0/Ki_x, 1.0/Ki_x)
		err_sum_y = constrain(err_sum_y + (0.0 - X_k[2]), -1.0/Ki_y, 1.0/Ki_y)
		err_sum_z = constrain(err_sum_z + (Z_d - X_k[0]), -1.0/Ki_z, 1.0/Ki_z)
		...
		states.linear.x 	= 0.0
		states.linear.y 	= 0.0
		states.linear.z 	= X_k[0]
		states.angular.x 	= X_k[1]
		states.angular.y 	= X_k[2]
		states.angular.z 	= X_k[3]
		pub1.publish(states);
